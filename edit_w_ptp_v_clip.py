...
class VaeInversion:
    ...
    def optimization(self, trainer, latents, image, num_inner_steps, num_epoch, epsilon):
        ...
        for epoch in range(num_epoch):
                ...
                for j in range(num_inner_steps):
                    ...
                    for attn_key in list(trainer.attention_store.keys()):
                        if attn_key in cross_attn_keys:
                            trainer.attention_store[attn_key] = [attn for attn in trainer.attention_store[attn_key] if attn.shape[1]==16*16]
                        else:
            ...
            with torch.no_grad():
                image_inv = ptp_utils_v.latent2image(ldm_stable.vae, latent_cur).squeeze()
...
def run_and_display(prompts, trainer, controller, latent=None, run_baseline=False, generator=None, verbose=True):
    if run_baseline:
        print("w.o. prompt-to-prompt")
        images, latent = run_and_display(prompts, trainer, EmptyControl(), latent=latent, run_baseline=False,
                                         generator=generator)
        print("with prompt-to-prompt")
    images, x_t = text2image_ldm_stable(ldm_stable, prompts, trainer, controller, latent=latent,
...
if __name__=="__main__":
    ...
    # w/ attn
    if args.w_attnloss:
        modeldir = "model_learnv"
        os.makedirs(modeldir, exist_ok=True)
        model_path = f'{modeldir}/model-inner{args.num_inner_steps}-epoch{args.num_epoch}-learnv-{args.idx}.pth'  # model name
    # w/o attn
    else:
        modeldir = "model_learnv_woattnloss"
        os.makedirs(modeldir, exist_ok=True)
        model_path = f'{modeldir}/model-inner{args.num_inner_steps}-epoch{args.num_epoch}-learnv-woattnloss-{args.idx}.pth'  # model name

    print(f'model name: {model_path}')
    vae_inversion = VaeInversion(ldm_stable)
    if args.is_train:
        image_path = ["./example_images/gnochi_mirror.jpeg",  # <1>
                      "./example_images/black and white dog playing red ball on black carpet.jpg",  # <2> https://unsplash.com/photos/b4BLUYKLzP4
                      "./example_images/black and white cat on floor.jpg",  # <3> https://unsplash.com/photos/93AcQpzcASE
                     ]
        prompts = ["a cat sitting next to a mirror",  # <1>
                   "black and white dog playing red ball on black carpet",  # <2>
                   "black and white cat on floor",  # <3>
                  ]
        image_path, prompts = image_path[args.idx-1:args.idx-1+args.batch_size], prompts[args.idx-1:args.idx-1+args.batch_size]

        if len(args.image_info) != 0:
            image_path, prompts = args.image_info
        ...
        prompt_gt = ["black and white dog playing red ball on black carpet"]


        # 3.
        '''
        image_path = ["./example_images/black and white cat on floor.jpg"]
        prompt_gt = ["black and white cat on floor"]
        '''

        if len(args.image_info) != 0:
            image_path, prompt_gt = args.image_info
        assert args.edit_type in ['StoreAttn', 'Replacement', 'Refinement']
        if args.edit_type == 'StoreAttn':
            prompt = []
            ...
            prompt = ["black and white tiger playing red ball on black carpet"]

            trainer.v_replace_steps = .6
            cross_replace_steps = {'default_': .6, }
            self_replace_steps = .8
            uncond_self_replace_steps = .5
            blend_word = ((('dog',), ( "tiger",)))
            eq_params = {"words": ("tiger",), "values": (2,)}

            '''
            prompt = ["black and white cat playing red ball on black carpet"]
            ...
            '''
            prompt = ["black and white robot playing red ball on black carpet"]

            trainer.v_replace_steps = .6
            cross_replace_steps = {'default_': .6, }
            self_replace_steps = .6
            uncond_self_replace_steps = .5
            blend_word = ((('dog',), ("robot",)))
            eq_params = {"words": ("robot",), "values": (2,)}
            '''

            # 3
            '''
            prompt = ["black and white robot on floor"]

            trainer.v_replace_steps = .4
            cross_replace_steps = {'default_': .7, }
            self_replace_steps = .5
            uncond_self_replace_steps = .0
            blend_word = ((('cat',), ("robot",)))
            eq_params = {"words": ("robot",), "values": (3,)}
            '''

            controller = make_controller(prompt_gt+prompt, True, cross_replace_steps, self_replace_steps, uncond_self_replace_steps, blend_word, eq_params)
        elif args.edit_type == 'Refinement':
            # 2
            prompt = ["watercolor painting of black and white dog playing red ball on black carpet"]

            trainer.v_replace_steps = .5
            cross_replace_steps = {'default_': .8, }
            self_replace_steps = .7
            uncond_self_replace_steps = .0
            blend_word = None
            eq_params = {"words": ("watercolor",), "values": (8, 2,)}

            # 3
            '''
            prompt = ["black and white crochet cat on floor"]

            trainer.v_replace_steps = .5
            cross_replace_steps = {'default_': .8, }
            self_replace_steps = .6
            blend_word = ((('cat',), ("cat",)))
            eq_params = {"words": ("crochet",), "values": (8,)}
            '''

            controller = make_controller(prompt_gt+prompt, False, cross_replace_steps, self_replace_steps, uncond_self_replace_steps, blend_word, eq_params)

        outdir = f"results/{prompt_gt[0] if args.edit_type == 'StoreAttn' else prompt[0]}"
        os.makedirs(outdir, exist_ok=True)

        (image_gt, image_rec), x_t = vae_inversion.eval_init(image_path, prompt_gt, trainer=trainer)

        image_inv, x_t = run_and_display(prompt_gt+prompt, trainer, controller, run_baseline=False, latent=x_t, verbose=False)
        if args.edit_type == 'StoreAttn':
            ...
            ptp_utils_v.view_images([image_inv[1].squeeze()], save_name=f'{outdir}/edited_img')
            ptp_utils_v.view_images([image_gt.squeeze(), image_rec.squeeze(), image_inv[0].squeeze(), image_inv[1].squeeze()], save_name=f'{outdir}/grid_img')
