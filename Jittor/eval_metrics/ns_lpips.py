...
def get_mask(bgmask_path):
    ...
    threshold = 0.5
    binary_tensor = (gray_tensor > threshold).cast("float32")
...
def main(args):
    ...
    source_imgs = jt.stack(source_imgs)
    mask_imgs = jt.stack(mask_imgs)
    target_imgs = jt.stack(target_imgs)

    src_bg, tgt_bg = source_imgs * mask_imgs, target_imgs * mask_imgs

    if lpips is None:
        raise RuntimeError('Install or provide a Jittor LPIPS implementation as jittor_lpips to run NS-LPIPS.')
