...
class NullInversion:
    ...
    def null_optimization(self, latents, num_inner_steps, epsilon):
        ...
        for i in range(NUM_DDIM_STEPS):
            uncond_embeddings = uncond_embeddings.clone().detach()  # clone() is deepcopy
            uncond_embeddings.requires_grad = True
            optimizer = nn.Adam([uncond_embeddings], lr=1e-2 * (1. - i / 100.))
    ...
    def invert_save(self, image_path: str, prompt: str, offsets=(0, 0, 0, 0), num_inner_steps=10, early_stop_epsilon=1e-5,
               verbose=False):
        self.init_prompt(prompt)
        image_gt = load_512(image_path, *offsets)
        image_rec, ddim_latents = self.ddim_inversion(image_gt)
        image_latents = []
        for i, latent in enumerate(ddim_latents):
            image_latents += [self.latent2image(latent)]
...
def text2image_ldm_stable(
    ...
    # image_latents = [null_inversion.latent2image(latents[0].unsqueeze(dim=0))]
...
if __name__=="__main__":
    ...
    parser.add_argument("--outdir", type=str, nargs="?", help="dir to write results to", default="Null_text_w_P2Plus_results")
    ...
    # null-text inversion
    (image_gt, image_enc), x_t, uncond_embeddings = null_inversion.invert(image_path, prompt, num_inner_steps=args.num_inner_steps, early_stop_epsilon=args.early_stop_epsilon, verbose=True)
    if edit_type == "StoreAttn":
        outdir = f'{outdir}/{prompt}'
        os.makedirs(outdir, exist_ok=True)

        controller = AttentionStore()
        image_inv, x_t = run_and_display([prompt], controller, run_baseline=True, latent=x_t, uncond_embeddings=uncond_embeddings)
        ...
        for tau_c in args.tau_c:
            for tau_s in args.tau_s:
                for tau_u in args.tau_u:
                    ...
                    images, _ = run_and_display(prompts, controller, run_baseline=True, latent=x_t, uncond_embeddings=uncond_embeddings)
                    ptp_utils.view_images([images[0], images[1]], save_name=f'{outdir}/(Inv+Nulltext)-tau_c{tau_c}-tau_s{tau_s}-tau_u{tau_u})')
