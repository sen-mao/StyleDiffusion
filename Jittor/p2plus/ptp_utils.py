...
def text_under_image(image: np.ndarray, text: str, text_color: Tuple[int, int, int] = (0, 0, 0)):
    ...
    if cv2 is None:
        ...
        draw.text(((w - text_w) // 2, h + (offset - text_h) // 2), text, fill=text_color, font=font)
...
def text2image_ldm(
    ...
    uncond_input = model.tokenizer([""] * batch_size, padding="max_length", max_length=77, return_tensors="np")
    uncond_embeddings = model.bert(jt.array(uncond_input.input_ids))[0]
    
    text_input = model.tokenizer(prompt, padding="max_length", max_length=77, return_tensors="np")
    text_embeddings = model.bert(jt.array(text_input.input_ids))[0]
...
def register_attention_control(model, controller):
    ...
    def attention_forward(self, place_in_unet):
        ...
        def execute(hidden_states, encoder_hidden_states=None, attention_mask=None, temb=None, scale=1.0, **kwargs):
            ...
            key = call_linear(self.to_k, context, scale)
            value = call_linear(self.to_v, context, scale)

            query = self.head_to_batch_dim(query)
            key = self.head_to_batch_dim(key)
            value = self.head_to_batch_dim(value)

            attn = self.get_attention_scores(query, key, attention_mask)
            attn = controller(attn, is_cross, place_in_unet)
