...
if __name__=="__main__":
    ...
    if edit_type == "StoreAttn":
        ...
        for i, seed in enumerate(args.seed):
            ...
            # Materialize one deterministic noise tensor shared by all tau_u values.
            seed_rng = np.random.RandomState(seed)
            seed_latent_np = seed_rng.standard_normal(
                (1, ldm_stable.unet.in_channels, 512 // 8, 512 // 8)
            ).astype(np.float32)
            for tau_c in args.tau_c:
                for tau_s in args.tau_s:
                    for tau_u in args.tau_u:
                        ...
                        images, _ = run_and_display(prompts, controller, run_baseline=True, latent=jt.array(seed_latent_np), generator=g_cpu)
